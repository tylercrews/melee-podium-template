...
TODO LIST:
* ‼️make it so that if you enter in character info, and then reduce the size of the bracket, that info is retained - like if you put in top 8 info and then accidentally click top 3 you lose 4-8 and that's annoying.
* make sure that startgg even if it suggests top 3 or top 4, always imports the top 8 info (or top 4 for doubles)
* I also want to use localstorage to retain your current work on the bracket in case you accidentally close out or click away, or click the link to manage favorites for a second. It's slightly annoying to refill all of that out. Just need to add a "Start Fresh/Clear Bracket Info" button to the top so that it doesn't keep old seeding and whatnot accidentally
* now that we have bracket phase link parsing in start.gg we have a unique issue - Top 8s for larger tournaments are registering as 8 person tournaments, and thus suggesting that you only do a top 3 for them. They're also not getting the correct entrant count. Maybe it'll be a magic number fix where it only checks if it's exactly 8, but I need to find a way for it to check the rest of the tournament and see if you inputted the "end" of the bracket so that it can grab the full details and make the right suggestion
