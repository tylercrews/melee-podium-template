...
- Format image settings include an eight-digit RGBA background color defaulting to `#00000000`, plus independent logarithmic logo and background size sliders from one tenth through ten times the source size, centered at one times. Backgrounds additionally support Scale to Width and Scale to Height, which resolve and serialize the exact multiplier required for the current source and output dimensions.
