...
- Structural preview images may use cached generalized backgrounds. A separate `Refresh Format Preview` action composites the user's current background color, selected/cropped background, logo size, and logo header position over a cached transparent structural render in the browser. Keep that user-asset refresh manual so image adjustments do not continuously trigger full podium renders.
- Track the complete settings-and-assets signature used by the last personalized Format preview. Visually emphasize the Refresh Format Preview action whenever the current signature differs, including after structural, header, image, crop, color, or size changes.
...
- Header Content includes serialized text settings. `Replace Base URLs With Icons` recognizes start.gg, YouTube, X, Bluesky, parry.gg, Challonge, and Twitch using the bundled `formatting_assets/website_icons/` assets. The metadata selector uses compact checkboxes and supports event, date, entrant/team count, tournament link, stream link, VOD link, TO X account, TO Twitch account, and TO Bluesky account, including select-all and deselect-all shortcuts.
...
- Left-clicking or keyboard-activating the Format preview opens the current cached or customized preview at a larger size in a modal dialog.
