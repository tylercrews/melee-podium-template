...
import { FighterOption, UserImage, UserImageCategory, deleteUserImage, getOptions, getUserImageUrl, listUserImages, uploadUserImage } from "./api";
...
function ImageLibrary({ category, images, selectedId, busyId, onSelect, onUpload, onDelete }: { category: UserImageCategory; images: UserImage[]; selectedId: string | null; busyId: string; onSelect: (image: UserImage) => void; onUpload: (category: UserImageCategory) => void; onDelete: (image: UserImage) => void }) {
  const atLimit = images.length >= MAX_IMAGES;
  return <section className="image-library" aria-labelledby={`${category}-heading`}>
    <div className="image-library__header"><span className="image-library__type-icon"><ImageTypeIcon category={category} /></span><div><h3 id={`${category}-heading`}>{categoryLabel(category)}</h3><p>{images.length} of {MAX_IMAGES} saved</p></div>{!atLimit && <button className="button button--outline image-library__upload" type="button" onClick={() => onUpload(category)}>+ Upload</button>}</div>
...
function Preview({ urls, ready, onContinue }: { urls: PreviewUrls; ready: boolean; onContinue: () => void }) {
  return <aside className="preview-column">
    <button className="preview-action" type="button" disabled={!ready} onClick={onContinue}><span>{ready ? "Continue to Format" : "Choose your images to continue"}</span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 5 7 7-7 7"/></svg></button>
...
export default function MakerApp() {
  ...
  const imagesComplete = Boolean(selected.tournament_logo && selected.background);
  const maxStep = imagesComplete ? 1 : 0;
  ...
  async function handleSignOut() { await signOutCurrentUser(); accountDialogRef.current?.close(); setActiveStep(0); }
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" type="button" onClick={() => { setPage("maker"); setActiveStep(0); }} aria-label="Melee Podium Maker home"><span className="brand__mark">M</span><span>Melee Podium Maker</span></button><div className="topbar__actions"><button className="button button--nav" type="button" onClick={() => setPage(page === "maker" ? "favorites" : "maker")}>{page === "maker" ? "Manage Favorite Entrants" : "Image Maker"}</button><button className={`account-button${user ? " account-button--signed-in" : ""}`} type="button" onClick={openAccount} aria-label={user ? `Account: ${displayName}` : "Sign in"}>{user?.photoURL ? <img src={user.photoURL} alt="" /> : <AccountIcon signedIn={Boolean(user)} />}<span className="account-button__dot" /></button></div></header>
    {page === "favorites" ? <div className="favorites-redesign"><FavoritesManagement favorites={favorites} fighters={fighters} renderCount={null} onChange={(nextFavorites) => setFavorites(saveFavorites(nextFavorites))} onBack={() => setPage("maker")} /></div> : <main className="maker-layout"><section className="workflow-column"><StepRail activeStep={activeStep} maxStep={maxStep} onSelect={setActiveStep} />{activeStep === 0 ? <section className="step-content"><div className="step-intro"><span className="eyebrow">Step 1 of 5</span><h1>Choose your images</h1><p>Select the artwork that sets the stage. You can fine-tune cropping and placement in the next step.</p></div>{!authReady ? <div className="loading-card">Checking your account…</div> : !user ? <SignInNotice onSignIn={openAccount} /> : <><div className="library-owner"><span>Signed in as</span><strong>{displayName}</strong></div>{libraryLoading ? <div className="loading-card">Loading your image library…</div> : <div className="image-libraries"><ImageLibrary category="tournament_logo" images={groupedImages.tournament_logo} selectedId={selected.tournament_logo} busyId={busyId} onSelect={selectImage} onUpload={openUpload} onDelete={handleDelete} /><ImageLibrary category="background" images={groupedImages.background} selectedId={selected.background} busyId={busyId} onSelect={selectImage} onUpload={openUpload} onDelete={handleDelete} /></div>}</>}{message && <p className="inline-message" role="alert">{message}</p>}<p className="future-note"><strong>Guest uploads:</strong> To do — stress-test browser memory limits before enabling unsaved local image uploads.</p></section> : <StubStep step={STEPS[activeStep]} />}</section><Preview urls={previewUrls} ready={imagesComplete} onContinue={() => setActiveStep(1)} /></main>}
